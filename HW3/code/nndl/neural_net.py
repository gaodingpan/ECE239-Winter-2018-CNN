...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    score_h1 = np.dot(X, W1.T) + b1 #size H * N
    output_reLu = reLu(score_h1)
    score_h2 = np.dot(output_reLu, W2.T) + b2
    scores = score_h2
    ...
    dh = dout.dot(W2).T
    dz = dh * (score_h1 > 0).T  # (N, H)
    
    grads['W2'] = (np.dot(output_reLu.T, dout) / N).T
  ...
  def predict(self, X):
    ...
    score_layer1 = X.dot(self.params['W1'].T) + self.params['b1']
    score_reLu = np.maximum(0, score_layer1)
