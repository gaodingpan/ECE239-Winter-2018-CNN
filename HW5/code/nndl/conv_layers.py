            ...
            for height in range(H_prime):
                for width in range(W_prime):
                    out[n, f, height, width] = np.sum(
                           x_pad[n, :, height * stride : height * stride + HH, 
...
def conv_backward_naive(dout, cache):
    ...
    for outh in range(out_height):
        for outw in range(out_width):
            xpad_sub = xpad[:, :, outh * stride : outh * stride + f_height, outw * stride : outw * stride + f_width]
            for f in range(F):
                dw[f, :, :, :] += np.sum(xpad_sub * (dout[:, f, outh, outw])[:, None, None, None], axis = 0)
            for n in range(N):
                dxpad[n, :, outh * stride : outh * stride + f_height, outw * stride : outw * stride + f_width] += np.sum(w[:, :, :, :] * (dout[n, :, outh, outw])[:, None, None, None], axis = 0)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            for outh in range(out_height):
                for outw in range(out_width):
                    out[n, c, outh, outw] = np.max(x[n, c, outh * stride:outh * stride + pool_height, outw * stride : outw * stride + pool_width])
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for outh in range(out_height):
                for outw in range(out_width):
                    x_pool = x[n, c, outh * stride : outh * stride + pool_height, outw * stride : outw * stride + pool_width]
                    max_x_pool = np.max(x_pool)
                    x_mask = (x_pool == max_x_pool)
                    dx[n, c, outh * stride : outh * stride + pool_height, outw * stride : outw * stride + pool_width] += dout[n, c, outh, outw] * x_mask
